...
__generate_cert(){
  ...
  if [ -f "${EXAMPLE_REQ}" ]; then
    ...
    # Then create a certificate signing request (CSR) for the domain:
    openssl req -config "${EXAMPLE_REQ}" \
                -new -sha256 \
                -key "${SERVERKEYFILE}" \
                -out "${_CERTS_DIR}/${DOMAIN}.csr" &2>/dev/null &&
    wait $!

    # Then to create a self-signed certificate from the CSR:
    openssl x509 -req -days 3650 \
                 -in "${_CERTS_DIR}/${DOMAIN}.csr" \
                 -signkey "${SERVERKEYFILE}" \
                 -out "${_CERTS_DIR}/${DOMAIN}.crt" &2>/dev/null &&
    wait $!
  fi
}
